...
DA is enabled if all drives are DA-capable. DA may be disabled after the volume is created by selecting menu:Storage[Volumes > View/Edit Settings > Advanced > Permanently disable data assurance]. If DA is disabled on a volume, it can not be re-enabled.
